...
if [ "$ctid" == "860" ] || [ "$ctid" == "870" ] || [ "$ctid" == "111590" ];then
continue
fi
...
echo "VPS $ctid is overloaded even after $(($looptimes * $loopsec)) seconds of removing VM IP $vmip Now the killing of the VPS and reboot would happen, but i interupted the script for testing purpose to see if it ignores 870 VM or not.. If this is regarding other than 870 vm, then take a look on VPS to reduce load."|mail -s "$(hostname): VPS $ctid was enligible for restart" $adminmail;exit
timeout 9 vzctl exec $ctid service mysql stop
